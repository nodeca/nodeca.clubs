// History of the edits made for club topics
//
...
module.exports = function (N, collectionName) {
  ...
  TopicHistory.statics.add = async function addHistory(changes, meta) {
    ...
    //
    let history_ids = await N.models.clubs.TopicHistory.find()
                                .where('topic').in(changes.map(x => x.new_topic?._id))
                                .select('topic _id')
                                .sort('_id')
                                .lean(true);
    ...
    let last_history_entry = _.keyBy(
      await N.models.clubs.TopicHistory.find()
                .where('_id').in(Object.values(history).map(x => x.last).filter(Boolean))
                .lean(true),
      'topic'
    );

    let bulk_history = N.models.clubs.TopicHistory.collection.initializeUnorderedBulkOp();
    ...
    for (let { old_topic, new_topic } of changes) {
      ...
      if (!prev &&
          ...
          meta.role === N.models.clubs.TopicHistory.roles.USER &&
