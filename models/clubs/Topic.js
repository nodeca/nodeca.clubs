...
module.exports = function (N, collectionName) {
  ...
  //
  Topic.index({ club: 1, st: 1, 'cache.last_post':    -1, _id: 1 });
  Topic.index({ club: 1, st: 1, 'cache_hb.last_post': -1, _id: 1 });

  // lookup _id by hid (for routing)
  Topic.index({ hid: 1 });

  // - count all topics from a user
  Topic.index({ 'cache.first_user': 1, st: 1 });
  ...
  Topic.pre('save', async function () {
    ...
    this.hid = await N.models.core.Increment.next('clubs_topic');
  });
  ...
  Topic.statics.updateCache = async function (topicID) {
    ...
    // Find last post
    let post = await N.models.clubs.Post.findOne()
    ...
    if (post.st === statuses.HB) {
      ...
      let post_visible = await N.models.clubs.Post.findOne()
                                  .where('topic').equals(topicID)
