// History of the edits made for club posts
//
...
module.exports = function (N, collectionName) {
  ...
  PostHistory.statics.add = async function addHistory(changes, meta) {
    ...
    //
    let history_ids = await N.models.clubs.PostHistory.find()
                                .where('post').in(changes.map(x => x.new_post?._id))
                                .select('post _id')
                                .sort('_id')
                                .lean(true);
    ...
    let last_history_entry = _.keyBy(
      await N.models.clubs.PostHistory.find()
                .where('_id').in(Object.values(history).map(x => x.last).filter(Boolean))
                .lean(true),
      'post'
    );

    let bulk_history = N.models.clubs.PostHistory.collection.initializeUnorderedBulkOp();
    ...
    for (let { old_post, new_post } of changes) {
      ...
      if (!prev &&
          ...
          meta.role === N.models.clubs.PostHistory.roles.USER &&
