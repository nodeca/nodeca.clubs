...
//             (non-club or unaccessible parts of the array will be empty)
...
module.exports = function (N, apiPath) {
  ...
  //
  N.wire.on(apiPath, { parallel: true }, async function club_posts_bookmarks_fetch(locals) {
    let bookmarks = locals.params.bookmarks
                        .filter(bookmark => bookmark.src_type === N.shared.content_type.CLUB_POST);
    ...
    await N.wire.emit(apiPath + ':club_posts', sub_env);
  ...
  // Find posts
  //
  N.wire.on(apiPath + ':club_posts', async function find_posts(locals) {
  ...
  //
  N.wire.on(apiPath + ':club_posts', async function check_permissions(locals) {
  ...
  // Sanitize results
  //
  N.wire.on(apiPath + ':club_posts', async function sanitize(locals) {
    if (!locals.sandbox.posts.length) return;
  ...
  // Fill results
  //
  N.wire.on(apiPath + ':club_posts', function fill_results(locals) {
    locals.results = {};
  ...
  // Fill users
  //
  N.wire.on(apiPath + ':club_posts', function fill_users(locals) {
    let users = {};
