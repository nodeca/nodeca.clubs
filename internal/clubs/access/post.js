...
// In:
//
// - params.topics - array of models.clubs.Post. Could be plain value
...
module.exports = function (N, apiPath) {
  ...
  N.wire.on('internal:common.access', async function check_post_access(access_env) {
    let match = N.router.matchAll(access_env.params.url).reduce(
      (acc, match) => match.meta.methods.get === 'clubs.topic' && (match.params.post_hid ? match : acc),
      null);
    ...
    await N.wire.emit('internal:clubs.access.post', access_env_sub);
  ...
  N.wire.before(apiPath, async function fetch_topics(locals) {
    ...
    let result = await N.models.clubs.Topic
                           .find()
                           .where('_id').in(ids)
                           .lean(true);
