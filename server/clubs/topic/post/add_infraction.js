...
module.exports = function (N, apiPath) {
  ...
  N.wire.before(apiPath, async function fetch_post(env) {
    env.data.post = await N.models.clubs.Post.findById(env.params.post_id)
                              .lean(true);
  ...
  N.wire.before(apiPath, async function fetch_topic(env) {
    env.data.topic = await N.models.clubs.Topic.findById(env.data.post.topic)
                              .lean(true);
  ...
  N.wire.before(apiPath, async function check_permissions(env) {
    let clubs_mod_can_add_infractions = await env.extras.settings.fetch('clubs_mod_can_add_infractions');

    if (!clubs_mod_can_add_infractions) throw N.io.FORBIDDEN;
