...
module.exports = function (N, apiPath) {
  ...
  N.wire.before(apiPath, function fetch_post_data(env) {
    return N.wire.emit('server:clubs.topic.post.edit.index', env);
  });
  ...
  N.wire.after(apiPath, async function fill_image_info(env) {
    await N.queue.club_post_images_fetch(env.data.post._id).postpone();
  });
  ...
  N.wire.after(apiPath, async function fetch_post(env) {
    ...
    await N.wire.emit('internal:clubs.post_list', env);
  });
};
