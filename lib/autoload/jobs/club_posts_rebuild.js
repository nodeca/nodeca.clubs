...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_club_posts_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'club_posts_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'club_posts_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(posts.map(p => String(p._id)), POSTS_PER_CHUNK)
                      .map(ids => N.queue.club_posts_rebuild_chunk(ids));
      ...
      async init() {
        let query = N.models.clubs.Post.countDocuments();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_post = await N.models.clubs.Post.findOne()
                                  .select('_id')
                                  .sort({ _id: 1 })
                                  .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_post = await N.models.clubs.Post.findOne()
                                  .select('_id')
                                  .sort({ _id: -1 })
                                  .lean(true);
    ...
    //
    N.queue.registerTask({
      name: 'club_posts_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        let start_time = Date.now();

        N.logger.info(`Rebuilding club posts ${ids[0]}-${ids[ids.length - 1]} - ${ids.length} found`);

        await N.wire.emit('internal:clubs.post_rebuild', ids);

        N.logger.info(`Rebuilding club posts ${ids[0]}-${ids[ids.length - 1]} - finished (${
    ...
    N.queue.on('task:progress:club_posts_rebuild', function (task_info) {
      N.live.debounce('admin.core.rebuild.club_posts', {
        uid:     task_info.uid,
    ...
    N.queue.on('task:end:club_posts_rebuild', function (task_info) {
      N.live.emit('admin.core.rebuild.club_posts', {
        uid:      task_info.uid,
        finished: true
      });
    });
