// Reindex all club topics (iterator started from admin interface)
//
'use strict';
...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_club_topics_search_rebuild() {

    N.queue.registerTask({
      name: 'club_topics_search_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'club_topics_search_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(topics.map(t => String(t._id)), TOPICS_PER_CHUNK)
                      .map(ids => N.queue.club_topics_search_update_by_ids(ids, { shadow: true }));
      ...
      async init() {
        ...
        // (arguments are ignored for club topic reindex only because
        ...
        let topics_count = await N.models.clubs.Topic.estimatedDocumentCount();
