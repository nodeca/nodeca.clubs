...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_club_topics_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'club_topics_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'club_topics_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(topics.map(t => String(t._id)), TOPICS_PER_CHUNK)
                      .map(ids => N.queue.club_topics_rebuild_chunk(ids));
      ...
      async init() {
        let query = N.models.clubs.Topic.countDocuments();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_topic = await N.models.clubs.Topic.findOne()
                                    .select('_id')
                                    .sort({ _id: 1 })
                                    .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_topic = await N.models.clubs.Topic.findOne()
                                    .select('_id')
                                    .sort({ _id: -1 })
                                    .lean(true);
    ...
    //
    N.queue.registerTask({
      name: 'club_topics_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        N.logger.info(`Rebuilding topic caches ${ids[0]}-${ids[ids.length - 1]} - ${ids.length} found`);

        await Promise.all(ids.map(id => N.models.clubs.Topic.updateCache(id)));
      }
    });


    N.queue.on('task:progress:club_topics_rebuild', function (task_info) {
      N.live.debounce('admin.core.rebuild.club_topics', {
        uid:     task_info.uid,
    ...
    N.queue.on('task:end:club_topics_rebuild', function (task_info) {
      N.live.emit('admin.core.rebuild.club_topics', {
        uid:      task_info.uid,
        finished: true
      });
    });
