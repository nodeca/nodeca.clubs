    ...
    N.queue.registerTask({
      ...
      async process() {
        try {
          ...
          try {
            await N.redis.renameAsync('views:clubs_topic:count', 'views:clubs_topic:count_tmp');
          } catch (__) {}

          let items = await N.redis.hgetallAsync('views:clubs_topic:count_tmp');

          await N.redis.delAsync('views:clubs_topic:count_tmp');
          ...
          await N.redis.zremrangebyscoreAsync('views:clubs_topic:track_last', '-inf', score);
        } catch (err) {
