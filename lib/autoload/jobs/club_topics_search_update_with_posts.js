...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_club_topics_search_update_with_posts() {

    N.queue.registerTask({
      name: 'club_topics_search_update_with_posts',
      pool: 'hard',
      ...
      init() {
        ...
        let tasks = ids.map(topic_id =>
          N.queue.club_posts_search_update_by_topic(topic_id)
        );

        tasks.unshift(N.queue.club_topics_search_update_by_ids(ids));
    ...
    // Task to index club posts from a selected topic (only used internally)
    //
    N.queue.registerTask({
      name: 'club_posts_search_update_by_topic',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'club_posts_search_update_by_topic',
