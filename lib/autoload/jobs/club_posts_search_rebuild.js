// Reindex all club posts (iterator started from admin interface)
//
'use strict';
...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_club_posts_search_rebuild() {

    N.queue.registerTask({
      name: 'club_posts_search_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'club_posts_search_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(posts.map(p => String(p._id)), POSTS_PER_CHUNK)
                      .map(ids => N.queue.club_posts_search_update_by_ids(ids, { shadow: true }));
