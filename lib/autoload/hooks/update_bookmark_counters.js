...
module.exports = function (N) {
  ...
  N.wire.after('server:users.bookmarks.destroy', async function update_club_post_bookmark_counters(env) {
    if (!env.data.bookmark) return;
    if (env.data.bookmark.src_type !== N.shared.content_type.CLUB_POST) return;
