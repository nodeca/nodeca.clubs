...
module.exports = function (N) {
  ...
  async function clubs_topic_gc_handler(userId, contentId, categoryId, max, currentCut) {
    ...
    let topic = await N.models.clubs.Topic.findById(contentId).lean(true);
  ...
  N.wire.after('init:models', { priority: 50 }, function marker_add_gc_handler() {
    N.models.users.Marker.registerGc('club_topic', clubs_topic_gc_handler);
  });
};
