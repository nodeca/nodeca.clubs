...
N.wire.on(module.apiPath + ':begin', function show_editor(data) {
  let $editor = N.MDEdit.show({
    draftKey: [ 'club_post_reply', N.runtime.user_hid, data.topic_hid, data.post_hid || '' ].join('_')
  });
  ...
  $editor
    .on('show.nd.mdedit', () => {
      let title = t(data.post_hid ? 'reply_post' : 'reply_topic', {
        topic_url: N.router.linkTo('clubs.topic', {
    ...
    .on('submit.nd.mdedit', () => {
      ...
      N.io.rpc('clubs.topic.post.reply', params).then(response => {
