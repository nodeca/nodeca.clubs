...
//   - hid:              club hid
//   - query:            search query
//   - type:             search type (club_posts, club_topics, etc.)
...
function load(start, direction) {
  ...
  return N.io.rpc('search.club_sole.results', Object.assign({}, search_params, {
    skip:   start,
...
N.wire.on('navigate.done:' + module.apiPath, function page_init(data) {
  ...
  if (search_params.query) {
    N.io.rpc('search.club_sole.results', Object.assign({}, search_params, {
      skip:   0,
      limit:  N.runtime.page_data.items_per_page
    })).then(res => {
      res.tabs.forEach(tab => {
        tab.link = N.router.linkTo('search.club_sole', {
